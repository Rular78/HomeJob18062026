...
def test_mask_account_card(card_info, expected, description) -> None:
    ...
    """
    assert mask_account_card(card_info) == expected, f"Ошибка в кейсе: {description} (входные данные: {card_info})"
...
def test_get_date(date_string, expected, description) -> None:
    ...
    """
    assert get_date(date_string) == expected, f"Ошибка в кейсе: {description} (входная дата: {date_string})"
