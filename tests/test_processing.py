...
from .fixtures.processing_data import FILTER_BY_STATE_TEST_CASES, SORT_BY_DATE_TEST_CASES # Импорт данных
...
def test_sort_by_date(data_list, is_descending, expected, description) -> None:
    """
    Параметризованный тест сортировки транзакций по дате.
    """
    assert sort_by_date(data_list, is_descending) == expected, (
        f"Ошибка в кейсе: {description}"
    )
